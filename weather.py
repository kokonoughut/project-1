...
def getWeather(window):
    ...
    try:
        ...
        cicon_image = ImageTk.PhotoImage(Image.open(BytesIO(cicon_data)))
        ...
        bg_image_data = Image.open('weather_tkinter\images/'+condition+'.jpg')
        ...
        cicons_label.configure(image=cicon_image)
        cicons_label.image = (cicon_image)
    except KeyError:
...
bg_image_data = Image.open('weather_tkinter\images/bg.jpg')
resized_image_data = bg_image_data.resize((800,800),Image.ANTIALIAS)
bg_image = ImageTk.PhotoImage(resized_image_data)
...
cicons_label= tk.Label(window,bg='#afafaf')
cicons_label.pack()
